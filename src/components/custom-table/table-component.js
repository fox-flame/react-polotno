...
const TableContent = observer(({ element, onCellClick, onMouseEnter, onMouseLeave }) => {
  const textareaRef = React.useRef(null);
  const [hoveredBorder, setHoveredBorder] = React.useState(null);
  const isResizing = React.useRef(false);
  const resizeDirection = React.useRef(null);
  const resizeStart = React.useRef({ x: 0, y: 0, width: 0, height: 0, index: -1 });
  ...
  React.useEffect(() => {
    if (!element.columnWidths || !element.rowHeights) {
      const initialColumnWidths = new Array(element.tableData[0].length).fill(defaultColumnWidth);
      const initialRowHeights = new Array(element.tableData.length).fill(defaultRowHeight);
      element.set({ 
  ...
  const startResize = (e, type, index) => {
    ...
    resizeStart.current = {
      ...
      index,
      sizes: type === 'column' ? 
        [...(element.columnWidths || new Array(element.tableData[0].length).fill(defaultColumnWidth))] : 
        [...(element.rowHeights || new Array(element.tableData.length).fill(defaultRowHeight))]
    };
  ...
  const handleResize = (e) => {
    ...
    const delta = resizeDirection.current === 'column' 
    ...
    const totalSize = resizeDirection.current === 'column' ? element.width : element.height;
  ...
  const getCellPosition = (rowIndex, colIndex) => {
    const columnWidths = element.columnWidths || new Array(element.tableData[0].length).fill(defaultColumnWidth);
    const rowHeights = element.rowHeights || new Array(element.tableData.length).fill(defaultRowHeight);
    
    const x = columnWidths.slice(0, colIndex).reduce((sum, width) => sum + width, 0);
    const y = rowHeights.slice(0, rowIndex).reduce((sum, height) => sum + height, 0);
    return { x, y };
  };
  ...
  const getResizeHandleStyle = (type, index) => {
    const position = type === 'column' ? getCellPosition(0, index + 1) : getCellPosition(index + 1, 0);
  ...
  React.useEffect(() => {
    if (hoveredBorder) {
      document.body.style.cursor = hoveredBorder.type === 'column' ? 'col-resize' : 'row-resize';
    } else {
  ...
  return (
    <>
            ...
            return (
              ...
              >
                <Rect
                  ...
                  fill={element.highlightedCell?.row === rowIndex && element.highlightedCell?.col === colIndex ? 'lightblue' : 'white'}
                />
                <Html>
                  <div
                    style={{
                  ...
                  >
                    <textarea
                      ref={textareaRef}
                      value={cell.text || ''}
                      onChange={(e) => handleTextChange(e, rowIndex, colIndex)}
                      style={{
                        width: '100%',
                        height: '100%',
                        border: 'none',
      ...
      {(element.columnWidths || new Array(element.tableData[0].length).fill(defaultColumnWidth)).map((width, colIndex) => {
        ...
        return (
          <Group key={`col-resize-${colIndex}`}>
            <Rect
              {...style}
              onMouseEnter={() => setHoveredBorder({ type: 'column', index: colIndex })}
              onMouseLeave={() => !isResizing.current && setHoveredBorder(null)}
              onMouseDown={(e) => startResize(e, 'column', colIndex)}
            />
            {(hoveredBorder?.type === 'column' && hoveredBorder.index === colIndex) && (
              <>
                <Rect
                  x={style.x + 14}
                  y={0}
      ...
      {(element.rowHeights || new Array(element.tableData.length).fill(defaultRowHeight)).map((height, rowIndex) => {
        ...
        return (
          <Group key={`row-resize-${rowIndex}`}>
            <Rect
              {...style}
              onMouseEnter={() => setHoveredBorder({ type: 'row', index: rowIndex })}
              onMouseLeave={() => !isResizing.current && setHoveredBorder(null)}
              onMouseDown={(e) => startResize(e, 'row', rowIndex)}
            />
            {(hoveredBorder?.type === 'row' && hoveredBorder.index === rowIndex) && (
              <>
                <Rect
                  x={0}
                  y={style.y + 14}
...
const TableComponent = observer(({ element, store }) => {
  ...
  const handleCellClick = (row, col) => {
    if (!element.tableData || !element.tableData[row] || !element.tableData[row][col]) {
      console.error('Invalid cell data:', { row, col, tableData: element.tableData });
      return;
    }
  ...
  return (
    <Group
      ...
      onDragEnd={stopDrag}
      onClick={() => store.selectElements([element])}
    >
      {/* Drag Handle */}
      <Group>
        <Rect
